...
export default function BorrowingShowLayout({
    ...
    const daysBorrowed = getDaysBorrowed(
        resource.borrowed_at,
        resource.returned_at
    );

    // Tentukan data peminjam berdasarkan role
    const peminjam =
        resource.role === "teacher" ? resource.teacher : resource.student;
    const peminjamType = resource.role === "teacher" ? "Guru" : "Siswa";
    const identifierField = resource.role === "teacher" ? "NIP" : "NISN";
    const identifierValue =
        resource.role === "teacher"
            ? resource.teacher?.nip
            : resource.student?.nisn;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div>
                        <h3 className="text-lg sm:text-xl font-semibold text-white mb-6 pb-3 border-b border-gray-700/50">
                            Informasi {peminjamType}
                        </h3>

                        <div className="flex items-center gap-4 mb-6">
                            {peminjam?.profile_picture ? (
                                <img
                                    src={`/storage/${peminjam.profile_picture}`}
                                    alt="Profile"
                            ...
                            <div>
                                ...
                                {resource.role === "student" && (
                                    <p className="text-gray-400">
                                        Kelas: {resource.student?.kelas}
                                    </p>
                                )}
                                {resource.role === "teacher" && (
                                    <p className="text-gray-400">
                                        Jabatan: {resource.teacher?.jabatan}
                                    </p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <DetailCard label="Email" value={peminjam?.email} />
                            <DetailCard label="No HP" value={peminjam?.no_hp} />
                        </div>
                    </div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-700/50">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <DetailCard
                            ...
                            value={
                                formatDate(resource.returned_at) ||
                                "Belum dikembalikan"
                            }
                        />
